class GreenBootstrapPantry < Pantry::Base
  def brand_name
    'Habanero'
  end
  
  def define_stacks
    can_stack 'Habanero::Brand', :scope => {:where => {:name => ['ActiveRecord', brand_name]}}
    can_stack 'Habanero::Variety', 
      :id_value_methods => [:name, :brand], 
      :scope => {
        :includes => :brand,
        :where => {:habanero_brands => {:name => ['ActiveRecord', brand_name]}}
      }
